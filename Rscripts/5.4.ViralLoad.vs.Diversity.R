...
for (i in 1:length(morder)){
    ...
    diversity<-sample[,c("File.name","Monkey","Week", "Tissue2","Coinfection")]
...
pdf("Output/MF_PID/Diversity.vs.VL.all.pdf", width = 12, height = 10)
do.call(grid.arrange, c(Plots, ncol=3))
dev.off()


# Calculate rolling average for VL pre- and post-Mtb infection (rolling average over 3 time points)
...
pdf("Output/MF_PID/VL.overtime.roll_background5_roll4.pdf", width = 12, height = 10)
...
for (i in 1:length(morder)){
    ...
    #original vl
    vlo<-vl
    roll<-rollmean(vlo$VL, k=4, align = "right",na.rm=T) 
    vlo$roll<-c(NA,NA,NA,roll)
    ...
    if (i>7){
        ...
        r<-rollmean(vl$VL, k=4, align = "right",na.rm=T) 
        vl$roll<-c(NA,NA,NA,r)
        vl$mtb="pre"
    }
    else{
        mtb<-tbs$tb[tbs$ids==monkeyID]
        pre_r3<- rollmean(vl$VL[vl$Week<mtb], k=4, align = "right",na.rm=T) 
        post_r3<-rollmean(vl$VL[vl$Week>=mtb],k=4, align = "right",na.rm=T) 
        
        vl$roll3<-c(NA,NA,NA,pre_r3, NA,NA,NA,post_r3)
        vl$mtb<-c(rep("pre", times=length(pre_r3)+3), rep("post", times=length(post_r3)+3))
        vl$mtb<-factor(vl$mtb, levels = c("pre","post"))
    }
...
pdf("Output/MF_PID/VL.overtime.roll_background4_roll4.pdf", width = 12, height = 10)
...
for (i in 1:length(morder)){
    ...
    if (i>7){
        ...
        r<-rollmean(vl$VL, k=3, align = "right",na.rm=T) 
        vl$roll<-c(NA,NA,r)
        vl$mtb="pre"
    }
    else{
        mtb<-tbs$tb[tbs$ids==monkeyID]
        pre_r3<- rollmean(vl$VL[vl$Week<mtb], k=3, align = "right",na.rm=T) 
        post_r3<-rollmean(vl$VL[vl$Week>=mtb],k=3, align = "right",na.rm=T) 
        
        vl$roll3<-c(NA,NA,pre_r3, NA,NA,post_r3)
        vl$mtb<-c(rep("pre", times=length(pre_r3)+2), rep("post", times=length(post_r3)+2))
        vl$mtb<-factor(vl$mtb, levels = c("pre","post"))
    }
...
pdf("Output/MF_PID/VL.overtime.roll_background4_roll3.pdf", width = 12, height = 10)
do.call(grid.arrange, c(Plots3, ncol=3))
dev.off()
