...
#bams<-list.files("Output/bam_PID/",pattern="^Run7.*.bam$")
...
for (i in 1:length(bams)){
    ...
    writeXStringSet(align, paste0("Output/PID_Con_Alignment/", fname, ".Alignment.fasta"), )
    ...
    write.csv(highSites,paste0("Output/Genotype/", fname, "variants.csv"))
...
files<-list.files("Output/Genotype/", pattern="variants.csv") #contains both 10 and 14 variants
...
genotypeCount<-data.frame(file=sub("variants.csv",'',files))

files<-files[files!="Run4_18variants.csv"]
...
for (i in 1: length(files)){
    ...
    #write.csv(gt2,paste0("Output/Genotype/Freq/",fname,".gtypeFreq.csv"))
    print(fname)
}
...
write.csv(genotypeCount,"Output/Genotype/Genotype_count_summary.csv")
...
ggsave("Output/Genotype/Freq.of.WT.Singleton.genotypes.pdf",height = 3,width = 8)    
