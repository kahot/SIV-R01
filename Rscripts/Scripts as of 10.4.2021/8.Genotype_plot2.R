...
## Genotype freq files
files<-list.files("Output/Genotype/Freq/", pattern="gtypeFreq.csv")
...
for (m in 1:length(monkeys2)){
    ...
    gtypes<-unique(gtypes) #429 unique genotypes for 1st monkey (21918)
    ...
    ggplot(data=plasma,aes(x=Week, y=Prop, fill=Genotype))+
        ...
        ggtitle(monkey)
    ggsave(paste0("Output/Genotype/Freq/",monkey,".Genotype_AreaStacked.pdf"),width = 9,height = 5)
...
for (i in 1:length(gtList)){
    ...
    colnames(df2)<-sites
    #df<-data.frame(cbind(df2, df))
    
    for (j in 1:n){
        dt<-data.frame(cbind(df[,c(j+1,j+2)], df2))
        dt<-dt[!is.na(dt[,1])|!is.na(dt[,2]),]
...
#95 41 10  6 10 10  8  6  5  1 

Com<-Common[Common$Total!=1,]
Com$Genotype[Com$Total%in% c(8,9,10)]

write.csv(Com,"Output/Genotype/MostCommonGenotype.csv")


G<-read.csv("Output/Genotype/MostCommonGenotype.csv", stringsAsFactors = F, row.names = 1)
do.call(rbind, str_split(before$type, '_and_'))


top20<-data.frame(Genotype=c("WT"))
for (m in 1:length(monkeys2)){
    monkey<-monkeys[m]
    Fq<-read.csv(paste0("Output/Genotype/Freq/",monkey,".GenotypeFreqNoDoubleton.csv"), row.names=1)
    #df<-df[order(df$Freq, decreasing = T),]
    
    Fq<-Fq[!(Fq$Genotype=="Singleton"|Fq$Genotype=="Doubleton"),]
    GT<-merge(GT,Fq[,1:2], by="Genotype", all=T)
    colnames()
    ...
    #find the common genotyes
    aggregate(df[2,3,])
        genotypes[[m]]<-unique(df$Genotype)
    names(genotypes)[m]<-moneky
    #top100
