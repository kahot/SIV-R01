...
#        df<-ovDF[[j]]
#        week<-sample$Week[j]
#        ave<-mean(df$freq.mutations.ref, na.rm=T)
#        div<-mean(df$freq.mutations, na.rm=T)
#        if (week<=17) col=cols[3]
#        else {col=cols2[1]}
...
#    }
#    pdf(paste0("Output/MF_PID/",monkey,".pdf"), width = 7, height = length(ovDF)*2)
...
ggplot(data=stock, aes(x=pos, y=freq.Ts.ref))+
    ylab("Mutation frequency")+xlab("")+
...
#Average MF (diversity) of stock
mean(stock$freq.mutations.ref, na.rm = T)
#0.0159661
...
for (i in 1:length(monkeyList)){
    ...
    pdf(paste0("Output/MF_PID/MVF.",monkey,".pdf"), width = 7, height = length(ovDF)*2)
    do.call(grid.arrange, c(Plot, ncol=1))
    dev.off()
}
...
cpos$occurence<-apply(cpos[,2:8], 1, function(x) length(x[x=="Y"]))
