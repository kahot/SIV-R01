...
for (i in 1:length(AAs)){
    ...
    write.csv(aa,paste0("Output/AA/Genotype/", fname, ".AA.10variants.csv"))
    print(fname)
}


## Calculate the genotype frequencies 
files<-list.files("Output/AA/Genotype/", pattern="\\.10variants.csv")
...
files<-files[files!="Run4_18.AA.10variants.csv"]
...
for (i in 1: length(files)){
    ...
    write.csv(gt3,paste0("Output/AA/Genotype.freq/",fname,".AA.10gtypeFreq.csv"))
    print(fname)
}
...
ggsave("Output/AA/Proprtion.Single.Double.10genotypes.pdf",height = 3,width = 8)    
...
freqfiles<-list.files("Output/AA/Genotype.freq/", pattern='10gtypeFreq.csv')
...
ggplot(data=GT2, aes(x=Count))+
    geom_histogram(fill="lightblue", bins=30, color="gray60")+
    xlab("Genotype appearance out of 109 samples")+ylab('')
ggsave("Output/AA/Genotype10.freq.zoom.pdf", width = 5, height = 3)    
