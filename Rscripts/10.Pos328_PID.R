...
for (i in 1:length(SIVFiles_overview)){ 
        ...
        names(OverviewP)[i]<-substr(paste(SIVFiles_overview[i]),start=1,stop=7)
}
...
# Stock is Run0_17
stock<-OverviewP[[which(names(OverviewP)=="Run0_17")]]
...
for (i in 1:length(monkeys2)){
    ...
    ovDF<-OverviewP[as.vector(sample$File.name)]
...
pdf("Output/MF_PID/Pos328_PID.pdf", width = 4, height=10)
