...
for ( i in 1:length(fList)){
    ...
    for (j in 1:length(sqnames)){
        ...
        if (nrow(seqs)>1){
            ...
            #Check indels for reverse 
            cigR<-seqs$cigar[2]
            gapr=0
            if (grepl("I|D",cigR)) {
                if (grepl("I",cigR)){
                    ins<-unlist(strsplit(cigR,"I"))
                    ins<-head(ins,-1)
                    for (k in 1:length(ins)){
                        m<-str_extract(ins[k], "(\\d+$)")
                        gapr<-gapr+as.numeric(m)
                    }
                }
                if (grepl("D",cigR)){
                    del<-unlist(strsplit(cigR,"D"))
                    del<-head(del,-1)
                    for (k in 1:length(del)){
                        d<-str_extract(del[k], "(\\d+$)")
                        gapr<-gapr-as.numeric(d)
                    }
                } 
            }       
        
            startF<-as.integer(seqs$Pos[1]) #start pos F
            startR<-as.integer(seqs$Pos[2])+gapr #start pos R
